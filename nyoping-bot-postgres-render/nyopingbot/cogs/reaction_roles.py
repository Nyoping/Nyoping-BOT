...
class ReactionRolesCog(commands.Cog):
    ...
    async def _fetch_rules_for_message(self, guild_id: int, message_id: int) -> list[dict[str, object]]:
        pool = getattr(self.bot, 'db_pool', None)
        if not pool:
            return []
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                """SELECT channel_id, emoji_key, add_role_ids, remove_role_ids
        ...
        for row in rows:
            item = {
                ...
                'emoji_key': _normalize_emoji_text(str(row['emoji_key'])),
                'add_ids': set(int(x) for x in (row['add_role_ids'] or [])),
                'rem_ids': set(int(x) for x in (row['remove_role_ids'] or [])),
            }
            out.append(item)
            self._rules[(int(guild_id), int(message_id), str(item['emoji_key']))] = {
                'channel_id': int(item['channel_id']),
                'add_ids': set(item['add_ids']),
                'rem_ids': set(item['rem_ids']),
            }
        return out

    async def _fetch_rule_from_db(self, guild_id: int, message_id: int, emoji_key: str) -> dict[str, object] | None:
        norm = _normalize_emoji_text(str(emoji_key))
        cached = self._rules.get((int(guild_id), int(message_id), norm))
        if cached:
            return cached
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                int(guild_id), int(message_id), norm,
            )
        if row:
            entry = {
                'channel_id': int(row['channel_id']),
            ...
            self._rules[(int(guild_id), int(message_id), norm)] = entry
            return entry

        rows = await self._fetch_rules_for_message(int(guild_id), int(message_id))
        for item in rows:
            if _normalize_emoji_text(str(item.get('emoji_key') or '')) == norm:
                return {
                    'channel_id': int(item['channel_id']),
                    'add_ids': set(int(x) for x in (item.get('add_ids') or set())),
                    'rem_ids': set(int(x) for x in (item.get('rem_ids') or set())),
                }

        self._rules.pop((int(guild_id), int(message_id), norm), None)
        return None
