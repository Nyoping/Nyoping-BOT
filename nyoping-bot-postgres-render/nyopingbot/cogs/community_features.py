...
async def _build_welcome_image_bytes(self, member: discord.Member, guild: discord.Guild, settings: dict, inviter) -> bytes | None:
    ...
    def _work():
        ...
        for layer in _iter_text_layers():
            rendered = _replace_vars(str(layer["template"]), member=member, guild=guild, inviter=inviter, mode="image")
            ...
            for i, line in enumerate(lines):
                ...
                draw.text((tx, int(layer["y"]) + i * line_gap), line, font=font, fill=str(layer["color"]))
...
async def _build_welcome_image_bytes(self, member: discord.Member, guild: discord.Guild, settings: dict, inviter) -> bytes | None:
    ...
    def _work():
        ...
        draw = ImageDraw.Draw(canvas)
        for layer in _iter_text_layers():
            text = _replace_vars(str(layer["template"]), member=member, guild=guild, inviter=inviter, mode="image")
            font = _safe_font(str(layer["font_name"]), int(layer["font_size"]))
            lines = _wrap_text_lines(draw, text, font, int(layer["box_width"]))
            line_gap = int(layer["font_size"]) + 8
            for i, line in enumerate(lines):
                bbox = draw.textbbox((0, 0), line, font=font)
                line_w = bbox[2] - bbox[0]
