...
class LevelingCog(commands.Cog):
    ...
    async def checkin(self, interaction: discord.Interaction):
        ...
        # 테스트 모드(출석 제한 OFF)에서는 bonus 설정이 0이어도 보너스가 실제로 보이도록
        # 최소 10XP/일을 임시 보너스로 적용합니다.
        ...
        if after_lv != before_lv:
            added_ids, removed_ids = await self._sync_roles_for_level(interaction.guild, interaction.user.id, after_lv, reason=f"레벨 변경(Lv.{before_lv}→Lv.{after_lv})")
            role_bits = []
            if added_ids:
                role_bits.append("추가: " + " ".join(f"<@&{i}>" for i in added_ids))
            if removed_ids:
                role_bits.append("제거: " + " ".join(f"<@&{i}>" for i in removed_ids))
            extra = "\n" + "\n".join(role_bits) if role_bits else ""
            await _send_notify_message(self.bot, interaction.guild.id, f"✅ {interaction.user.mention} 레벨 업! Lv.{before_lv} → Lv.{after_lv}{extra}")
