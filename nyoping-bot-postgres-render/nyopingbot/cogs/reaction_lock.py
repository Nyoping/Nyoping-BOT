...
class ReactionLockCog(commands.Cog):
    """특정 역할이 특정 메시지 반응을 누르지 못하게 막습니다.
    이미 눌러둔 반응을 사용자가 스스로 취소하는 것까지 완전히 막는 것은 Discord 구조상 불가능합니다.
    """
    ...
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent) -> None:
        # Discord API로는 '사용자가 반응을 취소하는 것'을 완전히 금지할 수 없고,
        # 사용자의 반응을 대신 다시 눌러줄 수도 없습니다.
        # 그래서 remove 이벤트는 기록만 남기고 별도 보정 동작을 하지 않습니다.
        try:
            if payload.guild_id is None:
                return
            if self.bot.user and int(payload.user_id) == int(self.bot.user.id):
            ...
            if blocked and self._member_has_blocked_role(member, blocked):
                log.info("Reaction lock: user removed own reaction guild=%s message=%s user=%s emoji=%s (cannot force restore)", payload.guild_id, payload.message_id, payload.user_id, payload.emoji)
        except Exception:
