...
CREATE TABLE IF NOT EXISTS reaction_blocks (
  ...
  PRIMARY KEY (message_id, blocked_role_id)
);

CREATE INDEX IF NOT EXISTS idx_reaction_blocks_guild
  ON reaction_blocks (guild_id);

CREATE INDEX IF NOT EXISTS idx_reaction_blocks_message
  ON reaction_blocks (message_id);
...
async def add_reaction_block(pool: asyncpg.Pool, guild_id: int, channel_id: int, message_id: int, blocked_role_id: int) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO reaction_blocks (guild_id, channel_id, message_id, blocked_role_id) VALUES ($1,$2,$3,$4) "
            "ON CONFLICT (message_id, blocked_role_id) DO UPDATE SET guild_id=EXCLUDED.guild_id, channel_id=EXCLUDED.channel_id",
            int(guild_id), int(channel_id), int(message_id), int(blocked_role_id)
        )

async def remove_reaction_block(pool: asyncpg.Pool, guild_id: int, message_id: int, blocked_role_id: int | None = None) -> int:
    async with pool.acquire() as conn:
        if blocked_role_id is None:
            r = await conn.execute(
                "DELETE FROM reaction_blocks WHERE guild_id=$1 AND message_id=$2",
                int(guild_id), int(message_id)
            )
        else:
            r = await conn.execute(
                "DELETE FROM reaction_blocks WHERE guild_id=$1 AND message_id=$2 AND blocked_role_id=$3",
                int(guild_id), int(message_id), int(blocked_role_id)
            )
    try:
        return int(str(r).split()[-1])
    except Exception:
