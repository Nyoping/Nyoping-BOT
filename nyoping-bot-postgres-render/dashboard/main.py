...
def _discord_application_id() -> str:
    return (
        ...
        or _env("DISCORD_APPLICATION_ID")
        or _env("APPLICATION_ID")
    ).strip()


def _discord_bot_permissions() -> str:
    value = _env("DISCORD_BOT_PERMISSIONS", "268823616").strip()
    return value or "268823616"


def _discord_install_scopes() -> str:
    raw = _env("DISCORD_INSTALL_SCOPES", "bot applications.commands").strip()
    scopes = [part.strip() for part in raw.replace(",", " ").split() if part.strip()]
    if "bot" not in scopes:
        scopes.append("bot")
    if "applications.commands" not in scopes:
        scopes.append("applications.commands")
    return " ".join(scopes)


def _discord_install_url(*, guild_id: int | None = None, lock_guild: bool = False) -> str:
