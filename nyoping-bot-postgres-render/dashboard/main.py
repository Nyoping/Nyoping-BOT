    ...
    for sql in candidates:
        ...
        for r in rows or []:
            try:
                ...
                rd = r
            try:
                gid_raw = rd.get("guild_id") if hasattr(rd, "get") else rd["guild_id"]
                gid = int(gid_raw)
            except Exception:
                continue
            if gid in seen:
            ...
            gname = ""
            try:
                gname = str((rd.get("guild_name") if hasattr(rd, "get") else rd["guild_name"]) or "")
            except Exception:
                gname = ""
            out.append({"guild_id": gid, "guild_name": gname})
