...
async def _list_guilds_for_admin(pool: asyncpg.Pool) -> list[dict[str, Any]]:
    ...
    # guild_name cache table may not exist in older installs, so keep it optional.
    candidates = [
    ...
    for sql in candidates:
        ...
        for r in rows or []:
            try:
                gid = int(r.get("guild_id"))
            except Exception:
                continue
            if gid in seen:
                continue
            seen.add(gid)
            out.append({"guild_id": gid, "guild_name": str(r.get("guild_name") or "")})
    out.sort(key=lambda x: int(x["guild_id"]), reverse=True)
    return out
...
async def admin_page(request: Request, guild_id: str | None = None, msg: str | None = None, rank_page: int = 1):
    ...
    return TEMPLATES.TemplateResponse(
        "admin.html",
        {
            ...
            "guild_name": next((str(g.get("guild_name") or g.get("guild_id")) for g in guilds if int(g.get("guild_id")) == int(gid)), (str(gid) if gid else "")),
