...
async def admin_set_level_role(
    ...
    guild_id: int = Form(...),
    level: int = Form(...),
    add_role_id: int = Form(...),
    remove_role_id: Optional[str] = Form(None),
) -> Response:
    if not _is_admin(request):
        return RedirectResponse(url="/", status_code=302)
    gid = int(guild_id)
    rid = int(remove_role_id) if (remove_role_id and remove_role_id.strip()) else None
    await _set_level_role(gid, int(level), int(add_role_id), rid)
