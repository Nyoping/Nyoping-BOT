...
def _build_test_welcome_image_bytes(form: dict[str, Any], member: dict[str, Any], guild_name: str) -> bytes | None:
    ...
    for layer in _pick_text_layers_from_form(form):
        ...
        font = _safe_font(layer["font_name"], layer["font_size"])
...
async def test_welcome_message(request: Request):
    ...
    try:
        ...
        if _to_bool(form.get("welcome_image_enabled")):
            image_bytes = _build_test_welcome_image_bytes(form, member, guild_name)
        ...
        if 200 <= resp.status_code < 300:
            return JSONResponse({"ok": True, "message": f"테스트 환영 메시지를 채널 {channel_id} 에 보냈어요."})
